...
func (a *AlarmActions) Snooze(ctx context.Context, alarmID string) error {
	...
	return a.c.doApp(ctx, http.MethodPost, path, nil, map[string]string{}, nil)
}
...
func (a *AlarmActions) Dismiss(ctx context.Context, alarmID string) error {
	...
	path := fmt.Sprintf("/v1/users/%s/alarms/%s/dismiss", a.c.UserID, alarmID)
	return a.c.doApp(ctx, http.MethodPost, path, nil, map[string]string{}, nil)
}
...
func (a *AlarmActions) DismissAll(ctx context.Context) error {
	...
	path := fmt.Sprintf("/v1/users/%s/alarms/active/dismiss-all", a.c.UserID)
	return a.c.doApp(ctx, http.MethodPost, path, nil, map[string]string{}, nil)
}
